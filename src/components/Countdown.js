        ...
        const updateTimers = () => {
            setTimers((prevTimers) =>
                prevTimers.map((timer) => {
                    ...
                    if (timeRemaining === 0 && timer.isRunning) {
                        clearInterval(intervalIds[timer.id]);

                        // Send browser notification
                        if (Notification.permission === "granted") {
                            new Notification("Event Reminder", {
                                body: `${timer.title} is happening now!`,
                            });
                        } else {
                            alert(`${timer.title} is happening now!`);
                        }
                            ...
                            >
                                ...
                                <div className="card-body d-flex flex-wrap justify-content-center">
                                    {timeRemaining.days > 0 && (
                                        <div className="container bg-light text-dark rounded m-2 text-center">
                                            <h1><strong>{timeRemaining.days}</strong></h1>
                                            <div>days</div>
                                        </div>
                                    )}
                                    <div className="container bg-light text-dark rounded m-2 text-center">
                                        <h1><strong>{timeRemaining.hours}</strong></h1>
                                        <div>hours</div>
                                    </div>
                                    <div className="container bg-light text-dark rounded m-2 text-center">
                                        <h1><strong>{timeRemaining.minutes}</strong></h1>
                                        <div>minutes</div>
                                    </div>
                                    <div className="container bg-light text-dark rounded m-2 text-center">
                                        <h1><strong>{timeRemaining.seconds}</strong></h1>
                                        <div>seconds</div>
                                    </div>
                                </div>
                                <button
